# 稳趋势扫描策略

<cite>
**本文引用的文件**   
- [README.md](file://README.md)
- [scan-stable.mjs](file://scan-stable.mjs)
- [smart-trend-decision.mjs](file://smart-trend-decision.mjs)
- [smart-trend-labels.mjs](file://smart-trend-labels.mjs)
- [smart-trend-monitor.mjs](file://smart-trend-monitor.mjs)
- [server.mjs](file://server.mjs)
- [scan-smart-signal.mjs](file://scan-smart-signal.mjs)
- [spot-symbol-check.mjs](file://spot-symbol-check.mjs)
- [data/smart-trend-push-mock.json](file://data/smart-trend-push-mock.json)
</cite>

## 更新摘要
**变更内容**   
- 新增现货验证系统集成，支持过滤无现货交易对的合约标的
- 增强趋势检测能力，引入严格趋势模式（strictTrend）
- 改进回撤控制算法，提供更精确的趋势完整性检查
- 优化双周期确认机制，支持灵活的周期组合配置

## 目录
1. [引言](#引言)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖关系分析](#依赖关系分析)
7. [性能与并发特性](#性能与并发特性)
8. [配置参数说明](#配置参数说明)
9. [使用场景与案例](#使用场景与案例)
10. [故障排查指南](#故障排查指南)
11. [结论](#结论)

## 引言
本文件为"右侧稳趋势"扫描策略的综合文档，面向希望理解并落地"右侧交易理论"的量化/半自动化交易者。策略围绕"右侧确认 + 回撤控制 + 双周期验证 + 评分模型 + 风险控制"展开，结合技术指标（MA、RSI、MACD）、聪明钱信号、市值过滤与飞书推送，形成可复用的稳健趋势发现流程。

**更新** 本次更新集成了新的现货验证系统和增强的趋势检测能力，进一步提升了策略的准确性和可靠性。

## 项目结构
- 策略核心实现位于 scan-stable.mjs，提供"右侧形态识别"和"右侧稳趋势筛选"两大能力。
- 决策层 smart-trend-decision.mjs 负责多源榜单合并、评分、连续性判断与输出卡片。
- 监控调度 smart-trend-monitor.mjs 负责定时任务、状态持久化、8am基准更新与汇总展示。
- 服务端 server.mjs 集成各模块，暴露 API 与定时任务，驱动稳趋势推送。
- 标签与格式化 smart-trend-labels.mjs 提供统一的数据展示与提示逻辑。
- 聪明钱信号 scan-smart-signal.mjs 提供净多空比等指标与分析。
- **新增** 现货验证系统 spot-symbol-check.mjs 提供现货交易对存在性检查和过滤功能。
- README.md 提供系统功能概览与配置项说明。

```mermaid
graph TB
A["server.mjs<br/>服务入口/定时任务"] --> B["scan-stable.mjs<br/>右侧稳趋势扫描"]
A --> C["smart-trend-monitor.mjs<br/>监控/调度/持久化"]
C --> D["smart-trend-decision.mjs<br/>决策/评分/连续性"]
C --> E["smart-trend-labels.mjs<br/>标签/格式化"]
C --> F["scan-smart-signal.mjs<br/>聪明钱信号"]
B --> G["spot-symbol-check.mjs<br/>现货验证系统"]
A --> H["data/smart-trend-push-mock.json<br/>推送样例数据"]
G --> I["Binance Spot API<br/>现货交易对信息"]
```

**图表来源**
- [server.mjs:1-120](file://server.mjs#L1-L120)
- [scan-stable.mjs:1-60](file://scan-stable.mjs#L1-L60)
- [smart-trend-monitor.mjs:1-60](file://smart-trend-monitor.mjs#L1-L60)
- [smart-trend-decision.mjs:1-40](file://smart-trend-decision.mjs#L1-L40)
- [smart-trend-labels.mjs:1-40](file://smart-trend-labels.mjs#L1-L40)
- [scan-smart-signal.mjs:1-40](file://scan-smart-signal.mjs#L1-L40)
- [spot-symbol-check.mjs:1-40](file://spot-symbol-check.mjs#L1-L40)
- [data/smart-trend-push-mock.json:1-20](file://data/smart-trend-push-mock.json#L1-L20)

**章节来源**
- [README.md:134-159](file://README.md#L134-L159)
- [server.mjs:540-590](file://server.mjs#L540-L590)

## 核心组件
- 右侧形态识别：基于 K 线序列计算 MA5/MA20、RSI6、MACD(DIF/DEA)、成交量变化，给出"是否处于右侧"及评分。
- **增强** 回撤控制：按时间周期自适应回看窗口，计算从近期高点回落幅度，限制最大回撤阈值，支持严格趋势模式。
- **增强** 双周期确认：在基础周期（默认 1h）满足右侧后，再对 1h 与 1d 进行回撤与趋势一致性校验。
- 评分模型：综合均线排列、价格相对位置、RSI区间、MACD金叉/多头、放量等因子加权计分。
- **新增** 现货验证系统：自动检查 Binance 现货交易对存在性，过滤无现货支持的合约标的。
- 风险控制：严格趋势模式拒绝阴跌/破位；双周期同时通过才纳入最终候选；连续出现次数统计用于排序与稳定性评估。

**章节来源**
- [scan-stable.mjs:8-139](file://scan-stable.mjs#L8-L139)
- [scan-stable.mjs:141-193](file://scan-stable.mjs#L141-L193)
- [scan-stable.mjs:249-289](file://scan-stable.mjs#L249-L289)
- [spot-symbol-check.mjs:1-106](file://spot-symbol-check.mjs#L1-L106)

## 架构总览
稳趋势扫描的整体流程如下：
- 服务端启动后加载环境变量与代理设置，初始化策略与监控模块。
- 定时任务触发时，调用 scanRightStable 扫描 Top N USDT 合约币种。
- **新增** 现货验证过滤：通过 filterSpotItems 函数过滤无现货交易对的合约标的。
- 对每个币种先做"右侧形态识别"，再通过"双周期回撤+趋势"校验。
- **增强** 趋势检测：支持严格趋势模式，提供更精确的趋势完整性检查。
- 结果经市值过滤、8am涨幅补充、大户/全网多空比增强后，生成飞书卡片推送。
- 监控模块维护历史状态、8am基准、评分与连续性，供决策层与面板展示使用。

```mermaid
sequenceDiagram
participant S as "server.mjs"
participant ST as "scan-stable.mjs"
participant SV as "spot-symbol-check.mjs"
participant M as "smart-trend-monitor.mjs"
participant D as "smart-trend-decision.mjs"
participant L as "smart-trend-labels.mjs"
participant SS as "scan-smart-signal.mjs"
S->>ST : 调用 scanRightStable(limit, maxDrawdownPct, dualTFConfirm, filterTF)
ST->>SV : filterSpotItems(rawSymbols)
SV-->>ST : 返回过滤后的现货支持标的
ST->>ST : checkCoinRightStable(含严格趋势模式)
ST-->>S : 返回 {symbol,label,score,drawdown,detail,...}
S->>M : 可选：批量附加聪明钱/资金费/8am涨幅
M->>SS : 获取聪明钱信号(可选)
SS-->>M : 返回 ratio/direction/score 等
M->>D : 构建决策/评分/连续性(可选)
D-->>M : 返回 action/watch/summary
M->>L : 格式化表格/标签
L-->>M : 返回渲染元素
S-->>S : 组装飞书卡片并推送
```

**图表来源**
- [server.mjs:668-781](file://server.mjs#L668-L781)
- [scan-stable.mjs:249-289](file://scan-stable.mjs#L249-L289)
- [spot-symbol-check.mjs:84-96](file://spot-symbol-check.mjs#L84-L96)
- [smart-trend-monitor.mjs:706-760](file://smart-trend-monitor.mjs#L706-760)
- [smart-trend-decision.mjs:556-640](file://smart-trend-decision.mjs#L556-640)
- [smart-trend-labels.mjs:203-224](file://smart-trend-labels.mjs#L203-L224)
- [scan-smart-signal.mjs:76-167](file://scan-smart-signal.mjs#L76-L167)

## 详细组件分析

### 右侧形态识别（单周期）
- 输入：K 线数组（OHLCV），默认最近 100 根。
- 指标计算：
  - MA5/MA20：简单移动平均，用于判断多头排列与支撑。
  - RSI6：短期动量，优选 50~80 区间避免超买区追高。
  - MACD：DIF/DEA，优先金叉或持续多头。
  - 成交量：近 3 根均量对比前 20 根均量，放量加分。
- 评分规则（示例权重）：
  - 均线多头排列或价格在 MA20 上方：+1~+2
  - RSI6 在合理区间：+1
  - MACD 金叉或多头：+1~+2
  - 放量：+1
- 输出：isRightSide(score>=3)、score、detail（含信号明细）。

```mermaid
flowchart TD
Start(["开始"]) --> GetK["读取K线序列"]
GetK --> CalcMA["计算MA5/MA20"]
CalcMA --> CheckMA{"价格>MA20且MA5>MA20?"}
CheckMA --> |是| ScoreA["+2 均线多排"]
CheckMA --> |否| CheckPrice{"价格>MA20?"}
CheckPrice --> |是| ScoreB["+1 价>MA20"]
CheckPrice --> |否| Next1["继续"]
ScoreA --> CalcRSI["计算RSI6"]
ScoreB --> CalcRSI
Next1 --> CalcRSI
CalcRSI --> CheckRSI{"50<RSI6<80?"}
CheckRSI --> |是| ScoreC["+1 RSI合理"]
CheckRSI --> |否| Next2["继续"]
ScoreC --> CalcMACD["计算MACD(DIF/DEA)"]
Next2 --> CalcMACD
CalcMACD --> CheckMACD{"DIF>DEA且金叉?"}
CheckMACD --> |是| ScoreD["+2 金叉"]
CheckMACD --> |否| CheckMACD2{"DIF>DEA多头?"}
CheckMACD2 --> |是| ScoreE["+1 多头"]
CheckMACD2 --> |否| Next3["继续"]
ScoreD --> VolCheck["比较近3根与前20根均量"]
ScoreE --> VolCheck
Next3 --> VolCheck
VolCheck --> VolPass{"放量>1.5x?"}
VolPass --> |是| ScoreF["+1 放量"]
VolPass --> |否| End
ScoreF --> SumScore["累计得分"]
End --> SumScore
SumScore --> Decide{"score>=3?"}
Decide --> |是| Right["标记为右侧"]
Decide --> |否| NotRight["非右侧"]
```

**图表来源**
- [scan-stable.mjs:8-139](file://scan-stable.mjs#L8-L139)

**章节来源**
- [scan-stable.mjs:8-139](file://scan-stable.mjs#L8-L139)

### 回撤控制与趋势完整性
- 回撤计算：取回看窗口内最高价与当前收盘价，计算回撤百分比。
- 回看窗口：根据周期自适应（1d→30，4h→36，其他→48）。
- **增强** 趋势完整性检查：
  - 宽松模式：价格不破 MA20，且在关键低点附近获得支撑。
  - **新增** 严格模式：拒绝 MA5<MA20、近3K走弱、连续阴线、跌破 MA20 一定比例。
- 通过条件：回撤≤阈值 且 趋势完整。

```mermaid
flowchart TD
S(["进入"]) --> TF["选择时间周期(1h/1d/4h)"]
TF --> Lookback["确定回看窗口长度"]
Lookback --> Peak["计算窗口内最高价"]
Peak --> DD["计算回撤=(Peak-Current)/Peak"]
DD --> DDOK{"回撤<=阈值?"}
DDOK --> |否| Fail["不通过"]
DDOK --> |是| Trend["趋势完整性检查(宽松/严格)"]
Trend --> TPass{"趋势完整?"}
TPass --> |否| Fail
TPass --> |是| Pass["通过"]
```

**图表来源**
- [scan-stable.mjs:50-89](file://scan-stable.mjs#L50-L89)
- [scan-stable.mjs:141-161](file://scan-stable.mjs#L141-L161)

**章节来源**
- [scan-stable.mjs:50-89](file://scan-stable.mjs#L50-L89)
- [scan-stable.mjs:141-161](file://scan-stable.mjs#L141-L161)

### 双周期确认机制
- 基础周期：默认 1h，先完成"右侧形态识别"。
- 双周期集合：[1h, 1d]，分别执行回撤与趋势检查。
- 判定：所有周期均通过才算"右侧稳趋势"。
- 输出：包含各周期回撤、趋势原因、是否通过等详情。

```mermaid
sequenceDiagram
participant Q as "查询器"
participant H as "1h周期"
participant D as "1d周期"
Q->>H : 拉取1h K线并计算回撤/趋势
H-->>Q : 返回{ddPct,trendOk,pass}
Q->>D : 拉取1d K线并计算回撤/趋势
D-->>Q : 返回{ddPct,trendOk,pass}
Q->>Q : allPass = h.pass && d.pass
Q-->>Q : 输出 isRightStable=allPass
```

**图表来源**
- [scan-stable.mjs:163-193](file://scan-stable.mjs#L163-L193)

**章节来源**
- [scan-stable.mjs:163-193](file://scan-stable.mjs#L163-L193)

### 现货验证系统
**新增** 现货验证系统是本次更新的核心功能，提供以下能力：

- **现货交易对检查**：基于 Binance /api/v3/exchangeInfo 接口，检查交易对是否存在且允许现货交易。
- **缓存机制**：6小时缓存现货交易对列表，减少API调用频率。
- **灵活过滤**：支持按交易对名称或基础资产进行过滤。
- **异步预热**：支持后台预热缓存，不影响主流程性能。

```mermaid
flowchart TD
Start(["开始"]) --> Load["加载现货交易对列表"]
Load --> Cache{"缓存是否有效?"}
Cache --> |是| UseCache["使用缓存数据"]
Cache --> |否| Fetch["调用Binance API"]
Fetch --> Parse["解析symbols数据"]
Parse --> Filter["过滤TRADING状态且允许现货交易"]
Filter --> Store["存储到缓存"]
UseCache --> Check["检查目标交易对"]
Store --> Check
Check --> Result{"是否有现货交易?"}
Result --> |是| True["返回true"]
Result --> |否| False["返回false"]
```

**图表来源**
- [spot-symbol-check.mjs:24-52](file://spot-symbol-check.mjs#L24-L52)
- [spot-symbol-check.mjs:84-96](file://spot-symbol-check.mjs#L84-L96)

**章节来源**
- [spot-symbol-check.mjs:1-106](file://spot-symbol-check.mjs#L1-L106)

### 评分模型与连续性
- 评分维度：
  - 多榜共振（来源数量）
  - 右侧形态命中
  - 涨跌幅排名（涨幅/跌幅/成交额）
  - 聪明钱方向与变化幅度
  - 资金费率变化（辅助）
- 连续性：
  - 记录上次出现时间与方向，若反向需连续2小时确认，避免频繁反手。
  - 强度变化：分数提升/下降影响"加强/减弱"状态。
- 输出：action（立刻关注）、watch（继续观察）、invalidated（已失效）。

```mermaid
classDiagram
class DecisionItem {
+string symbol
+string label
+string tradeView
+number score
+string confidence
+string status
+number streak
+number[] sources
+number change24h
+number change8am
+number ratio
+number ratioDeltaPct
+number ratio8amDeltaPct
+string[] reasons
+boolean hasSpot
}
class Continuity {
+string status
+string statusLabel
+number streak
+string pendingSide
+number pendingStreak
+number firstSeenAt
+number lastSeenAt
}
DecisionItem --> Continuity : "关联状态"
```

**图表来源**
- [smart-trend-decision.mjs:424-472](file://smart-trend-decision.mjs#L424-L472)
- [smart-trend-decision.mjs:340-403](file://smart-trend-decision.mjs#L340-L403)

**章节来源**
- [smart-trend-decision.mjs:282-313](file://smart-trend-decision.mjs#L282-313)
- [smart-trend-decision.mjs:340-403](file://smart-trend-decision.mjs#L340-L403)
- [smart-trend-decision.mjs:556-640](file://smart-trend-decision.mjs#L556-640)

### 与外部模块的数据交互与输出格式
- 输入：
  - 币安合约 K 线、24h 行情、聪明钱信号、资金费率、市值数据。
  - **新增** 现货交易对信息（来自 Binance Spot API）。
  - 用户配置：推送间隔、扫描数量、回撤阈值、是否开启双周期等。
- 处理：
  - **新增** 现货验证过滤：自动排除无现货交易对的合约标的。
  - 市值过滤（默认上限 50 亿美元，可配置）。
  - TradFi 股票相关币种排除。
  - 8am 基准价格与涨幅计算。
- 输出：
  - 飞书卡片（表格+Markdown），字段包括：币种、价格、8am/24h涨跌、评分、回撤、市值、大户/全网多空比、连续推荐次数等。
  - **新增** 现货支持标记：在显示中标记有现货交易的标的。
  - 决策摘要：重点关注、继续观察、已失效列表。

**章节来源**
- [README.md:134-159](file://README.md#L134-L159)
- [server.mjs:646-781](file://server.mjs#L646-781)
- [smart-trend-monitor.mjs:706-760](file://smart-trend-monitor.mjs#L706-760)
- [spot-symbol-check.mjs:84-96](file://spot-symbol-check.mjs#L84-L96)
- [data/smart-trend-push-mock.json:1-20](file://data/smart-trend-push-mock.json#L1-L20)

## 依赖关系分析
- 外部依赖：
  - 币安合约 REST API（fapi.binance.com）
  - **新增** 币安现货 REST API（api.binance.com/api/v3/exchangeInfo）
  - 聪明钱信号 API（bapi.futures.smart-money）
  - 市值数据（CoinMarketCap Pro 或 CoinGecko 回退）
  - 飞书 Webhook 推送
- 内部依赖：
  - server.mjs 聚合各模块，管理定时任务与缓存。
  - smart-trend-monitor.mjs 负责状态持久化与 8am 基准。
  - smart-trend-decision.mjs 负责多源合并与评分。
  - smart-trend-labels.mjs 提供统一展示逻辑。
  - scan-smart-signal.mjs 提供聪明钱信号解析。
  - **新增** spot-symbol-check.mjs 提供现货验证功能。

```mermaid
graph LR
Server["server.mjs"] --> Stable["scan-stable.mjs"]
Server --> Monitor["smart-trend-monitor.mjs"]
Monitor --> Decision["smart-trend-decision.mjs"]
Monitor --> Labels["smart-trend-labels.mjs"]
Monitor --> SmartSignal["scan-smart-signal.mjs"]
Stable --> SpotCheck["spot-symbol-check.mjs"]
Monitor --> SpotCheck
Server --> Feishu["飞书Webhook"]
Server --> Binance["币安合约API"]
Server --> BinanceSpot["币安现货API"]
Server --> MarketCap["市值API(CMC/CG)"]
```

**图表来源**
- [server.mjs:1-120](file://server.mjs#L1-L120)
- [smart-trend-monitor.mjs:1-60](file://smart-trend-monitor.mjs#L1-L60)
- [smart-trend-decision.mjs:1-40](file://smart-trend-decision.mjs#L1-L40)
- [smart-trend-labels.mjs:1-40](file://smart-trend-labels.mjs#L1-L40)
- [scan-smart-signal.mjs:1-40](file://scan-smart-signal.mjs#L1-L40)
- [spot-symbol-check.mjs:1-40](file://spot-symbol-check.mjs#L1-L40)

**章节来源**
- [server.mjs:540-590](file://server.mjs#L540-L590)

## 性能与并发特性
- 并发扫描：
  - pmap 工具函数控制并发度，默认 3，避免过多请求导致限流。
- 缓存与去重：
  - ticker/24hr 短 TTL 缓存，同轮次并发去重。
  - **新增** 现货交易对列表缓存（6小时），减少API调用。
  - 聪明钱信号接口具备熔断与节流保护。
- 网络重试：
  - 币安 API 代理封装带指数退避重试。
- 磁盘持久化：
  - 监控状态与决策状态异步落盘，降低 IO 阻塞。

**章节来源**
- [scan-stable.mjs:195-206](file://scan-stable.mjs#L195-L206)
- [server.mjs:84-105](file://server.mjs#L84-L105)
- [scan-smart-signal.mjs:57-74](file://scan-smart-signal.mjs#L57-L74)
- [smart-trend-monitor.mjs:111-137](file://smart-trend-monitor.mjs#L111-L137)
- [spot-symbol-check.mjs:24-52](file://spot-symbol-check.mjs#L24-L52)

## 配置参数说明
- 推送与扫描
  - STABLE_PUSH_HOURS：推送间隔（小时）
  - STABLE_PUSH_ENABLED：是否启用推送
  - STABLE_SCAN_LIMIT：扫描币种数量（默认 200）
- 风控与阈值
  - STABLE_MAX_DRAWDOWN：最大回撤阈值（默认 0.30，即 30%）
  - MAX_MARKET_CAP_USD：市值上限（美元），超过则不监控/不推送；设为 0 关闭
- **新增** 现货验证配置
  - FILTER_SPOT_ONLY：是否仅保留有现货交易的标的（默认 false）
- 其他
  - PORT：Web 面板端口
  - CMC_API_KEY：CoinMarketCap Pro Key（未配置回退 CoinGecko）
  - FEISHU_WEBHOOK：飞书机器人 Webhook URL

**章节来源**
- [README.md:142-159](file://README.md#L142-L159)
- [server.mjs:540-590](file://server.mjs#L540-L590)
- [spot-symbol-check.mjs:11-12](file://spot-symbol-check.mjs#L11-L12)

## 使用场景与案例
- 稳健趋势机会识别：
  - 步骤：
    1) 设定回撤阈值（如 30%）与双周期确认。
    2) **新增** 启用现货验证过滤，确保标的具有现货流动性。
    3) 扫描 Top 200 USDT 合约，筛选右侧形态（MA/RSI/MACD/放量）。
    4) 对 1h 与 1d 进行回撤与趋势一致性校验。
    5) 附加聪明钱信号与资金费率，生成评分与连续性。
    6) 输出飞书卡片，按连续推荐次数与评分排序。
  - 典型特征：
    - 价格在 MA20 上方，MA5 与 MA20 多头排列。
    - RSI6 处于 50~80 区间，MACD 金叉或持续多头。
    - 回撤控制在阈值以内，1d 趋势同样健康。
    - 聪明钱偏多且 1h/8am 变化显著。
    - **新增** 具有现货交易支持，流动性有保障。
- 案例分析（参考推送样例数据结构）：
  - 字段含义：symbol、label、price、chg8am、chg24h、score、dd、mc、whale、streak。
  - **新增** hasSpot：表示该标的是否具有现货交易支持。
  - 解读要点：
    - score 越高，右侧形态越完整。
    - dd 越低，回撤风险越小。
    - whale 表示大户比与全网比的比值，>1 表示大户更偏多。
    - streak 表示连续被推荐的次数，体现稳定性。
    - **新增** hasSpot 为 true 的标的可能具有更好的流动性和市场深度。

**章节来源**
- [data/smart-trend-push-mock.json:1-20](file://data/smart-trend-push-mock.json#L1-L20)
- [server.mjs:744-770](file://server.mjs#L744-770)
- [spot-symbol-check.mjs:84-96](file://spot-symbol-check.mjs#L84-L96)

## 故障排查指南
- 推送失败：
  - 检查 FEISHU_WEBHOOK 是否配置正确。
  - 查看限流错误信息，必要时调整推送频率。
- 数据异常：
  - 币安 API 报错：检查代理节点与网络连通性。
  - 聪明钱信号限流熔断：等待熔断时间后重试。
  - **新增** 现货API调用失败：检查网络连接，系统会自动降级处理。
- 市值过滤误杀：
  - 确认 MAX_MARKET_CAP_USD 配置，必要时调高或关闭过滤。
- **新增** 现货验证问题：
  - 检查 FILTER_SPOT_ONLY 配置，确认是否需要启用现货过滤。
  - 查看现货交易对缓存是否成功加载。
- 端口占用：
  - 使用 dev:restart 自动清理旧进程，或手动结束占用端口的进程。

**章节来源**
- [server.mjs:599-644](file://server.mjs#L599-644)
- [scan-smart-signal.mjs:42-55](file://scan-smart-signal.mjs#L42-L55)
- [README.md:40-53](file://README.md#L40-53)
- [spot-symbol-check.mjs:102-106](file://spot-symbol-check.mjs#L102-L106)

## 结论
"右侧稳趋势"策略以"右侧确认 + 回撤控制 + 双周期验证 + 评分模型 + 风险控制"为核心，结合聪明钱信号与市值过滤，能够在波动市场中稳定地筛选出稳健的趋势交易机会。**本次更新集成的现货验证系统和增强的趋势检测能力**，进一步提升了策略的准确性和可靠性。

通过合理的配置与持续的监控，可有效降低追高风险，提高胜率与盈亏比。建议在实际使用中结合个人风险偏好与资金管理策略，逐步优化阈值与评分权重。**新增的现货验证功能**确保了推荐标的具有良好的流动性，而**严格趋势模式**则为保守型交易者提供了更安全的筛选标准。